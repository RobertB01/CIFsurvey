...
class spec_class {
    ...
    /** Discrete variable "conveyor1.pos". */
    conveyor1_pos_;

    /** Discrete variable "conveyor2.pos". */
    conveyor2_pos_;
    ...
    execOnce(newTime) {
        ...
        while (true) {
            ...
            // Event "conveyor1.exit1".
            edgeExecuted |= this.execEdge0();

            // Event "conveyor2.exit2".
    ...
    /**
     * Execute code for edge with index 0 and event "conveyor1.exit1".
     *
     * @return 'true' if the edge was executed, 'false' otherwise.
     */
    execEdge0() {
        var guard = (specUtils.equalObjs(spec.conveyor1_pos_, 4)) && (specUtils.equalObjs(spec.conveyor2_pos_, -(1)));
        ...
        spec.conveyor2_pos_ = spec.conveyor1_pos_;
        spec.conveyor1_pos_ = 0;
    ...
    /**
     * Execute code for edge with index 1 and event "conveyor2.exit2".
     *
     * @return 'true' if the edge was executed, 'false' otherwise.
     */
    execEdge1() {
        var guard = specUtils.equalObjs(spec.conveyor2_pos_, 7);
    ...
    /**
     * Execute code for edge with index 2 and event "move".
     *
     * @return 'true' if the edge was executed, 'false' otherwise.
     */
    execEdge2() {
        ...
        if ((spec.conveyor1_pos_) < (4)) {
            spec.conveyor1_pos_ = specUtils.addInt(spec.conveyor1_pos_, 1);
        } else if ((spec.conveyor1_pos_) >= (4)) {
            spec.conveyor1_pos_ = spec.conveyor1_pos_;
        }
        if (((spec.conveyor2_pos_) >= (0)) && ((spec.conveyor2_pos_) < (7))) {
            spec.conveyor2_pos_ = specUtils.addInt(spec.conveyor2_pos_, 1);
        } else if (!(((spec.conveyor2_pos_) >= (0)) && ((spec.conveyor2_pos_) < (7)))) {
            spec.conveyor2_pos_ = spec.conveyor2_pos_;
        }
    ...
    getStateText() {
        ...
        state += specUtils.fmt(', conveyor1.pos=%s', specUtils.valueToStr(spec.conveyor1_pos_));
        state += specUtils.fmt(', conveyor2.pos=%s', specUtils.valueToStr(spec.conveyor2_pos_));
        return state;
    }
